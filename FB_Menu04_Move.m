...
while(true_feedback);
    ...
    for k=1:(sec/ifi)
        ...
        Screen('FillRect', window, freq_long(4,k) ,pos_block4);
    ...
    if (time >= sec + wait_acquisition) 
        ...
        if ( ~isempty(devents) ) 
            ...
            % Switch to the according menu
                switch predTgt
                    case 1 % Freq. of block 1 --> go up
                        sendEvent('navigate','up');
                        FB_Menu04_Move
                    case 2 % Freq. of block 2 --> go to the left
                        sendEvent('navigate','left');
                        FB_Menu04_Move
                    case 3 % Freq. of block 3 --> go to the right
                        sendEvent('navigate','right');
                        FB_Menu04_Move
                    case 4 % Freq. of block 4 --> go to the back
                        sendEvent('navigate','down');
                        FB_Menu04_Move
